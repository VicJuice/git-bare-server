...
# ENV HOME /root
RUN /etc/my_init.d/00_regen_ssh_host_keys.sh -f && rm -f /etc/service/sshd/down
# Uncomment to Enable SSHD
#RUN rm -rf /etc/service/sshd /etc/my_init.d/00_regen_ssh_host_keys.sh           
# Uncomment to Disable SSHD
CMD ["/sbin/my_init"]
...
RUN groupadd -g 987 git && useradd -g root -u 987 -d /git -m -r -s /usr/bin/git-shell git
